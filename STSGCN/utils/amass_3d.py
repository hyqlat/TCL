...
class Datasets(Dataset):
    ...
    def __init__(self, data_dir,input_n, output_n, skip_rate, actions=None, split=0):
        ...
        :param sample_rate:
        """
        self.path_to_data = data_dir + '/' + 'amass/'  
        self.split = split
        self.in_n = input_n
        self.out_n = output_n
...
def mpjpe_error_test(batch_pred,batch_gt):
    #assert batch_pred.requires_grad==True
    #assert batch_gt.requires_grad==False
    #B T C 3
    ...
    return torch.mean(torch.mean(torch.norm(batch_gt-batch_pred,2,3), dim=-1), dim=0)#T
